...
int     WhichHeliMode;          // whether inside or outside the helicopter    
bool    Freeze,Light0On, Light1On, Light2On, Light3On;
...
void	Resize(int, int);
void	Visibility(int);
//void   createBladesAt(int, int, float, float, float);
...
Display()
{
	...
	glLightModelfv(GL_LIGHT_MODEL_AMBIENT, MulArray3(.4f, White));
	...
	glPushMatrix();
	    SetBlob(0, 0, 1);
		glTranslatef(0, 1, z-0.6);
		MjbSphere(0.1,50,50);
		SetPointLight(GL_LIGHT0, 0, 1, z-0.6, 0, 0, 1);
	glPopMatrix();
...
InitLists()
{
	...
	glPushMatrix();
		...
		Texture = BmpToTexture("worldtex.bmp", &width, &height);
			...
			glPushMatrix();
			    SetMaterial(0, 0.7, 0, 8);
				glShadeModel(GL_SMOOTH);
			    glTranslatef(0, 2, -4.5);
				glutSolidTorus(1,3.5,100, 100);
			glPopMatrix();
			
			glPushMatrix();
			    glTranslatef(1, 5, -12);
				SetBlob(1, 1, 1);
				MjbSphere(0.05, 140, 140);
				SetPointLight(GL_LIGHT1, 1, 5, -11.9, 1, 1, 1);			
			glPopMatrix();
			...
			SetBlob(1, 1, 1);
			MjbSphere(0.05, 140, 140);
			SetPointLight(GL_LIGHT2, 0.5, -4, 4, 1, 1, 1);
			glPopMatrix();
			...
			glPushMatrix();
				...
				SetBlob(1, 0, 0);
				MjbSphere(0.05, 140, 140);
				SetSpotLight(GL_LIGHT3, 0, -0.5, -0.65, -1, 1, -1, 1, 0, 1);		
			glPopMatrix();
